...
function StepDetails({
  ...
  onBack, onNext, isStaff, floorPrice,
}: {
  laptop: StockLaptop
  details: SaleDetails
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <p style={{ color: 'var(--text-3)', fontSize: 12 }}>{specsLine(laptop.specs ?? {}) || '—'} \xb7 S/N {laptop.imei ? `…${laptop.imei.slice(-6)}` : 'No serial'}</p>
        </div>
      ...
      <div>
        ...
        {floorPrice && floorPrice > 0 && hasPrice && price < floorPrice && (
          <div style={{
            marginTop: 10,
            ...
            <p style={{ color: 'var(--danger)', fontSize: 13, fontWeight: 500 }}>
              Sale price is below the floor price ({fmtRs(floorPrice)}). This sale will be flagged for owner review.
            </p>
          </div>
        )}
      </div>
...
function SuccessScreen({
  profit, exchanged, onAnother, isStaff, receiptData,
}: {
  profit: number
  exchanged: boolean
  onAnother: () => void
  isStaff?: boolean
  receiptData: import('@/lib/utils/receipt').ReceiptData | null
}) {
      ...
      {receiptData && (
        <div style={{ marginBottom: 20 }}>
          <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'var(--info-bg)', border: '1px solid var(--info-border)', borderRadius: 8, padding: '10px 18px', color: 'var(--info)', fontWeight: 600, fontSize: 13, cursor: downloading ? 'not-allowed' : 'pointer', opacity: downloading ? 0.7 : 1 }}
          >
...
function NewSaleInner() {
  ...
  // Look up floor price whenever a laptop is selected
  useEffect(() => {
    if (!selected || !shop) { setFloorPrice(null); return }
    const supabase = createClient()
    supabase
      .from('model_pricing')
      .select('floor_price')
      .eq('shop_id', shop.id)
      .eq('model', selected.model)
      .eq('condition', selected.condition ?? 'used')
      .maybeSingle()
      .then(({ data }) => setFloorPrice(data?.floor_price ?? null))
  }, [selected, shop])
  ...
  const handleConfirm = async () => {
    ...
    const salePrice = parseFloat(details.sale_price)
    const isBelow = floorPrice !== null && floorPrice > 0 && salePrice < floorPrice
    fd.append('below_floor', isBelow ? 'true' : 'false')
    if (floorPrice !== null && floorPrice > 0) fd.append('floor_price_at_sale', String(floorPrice))
  ...
  return (
    <div style={{ maxWidth: 680, marginInline: 'auto' }}>
      ...
      {doneProfit !== null && !showMarketPrompt ? (
        <SuccessScreen
          profit={doneProfit}
          exchanged={exchange.enabled}
          onAnother={reset}
          isStaff={isStaff}
          receiptData={selected && shop ? {
            shopName: shop.name,
            whatsappNumber: shop.whatsapp_number,
            soldAt: new Date().toISOString(),
            brand: selected.brand,
            model: selected.model,
            processor: String((selected.specs ?? {}).processor ?? '') || null,
            ram: String((selected.specs ?? {}).ram ?? '') || null,
            storage: String((selected.specs ?? {}).storage ?? '') || null,
            imei: selected.imei,
            salePrice: parseFloat(details.sale_price) || 0,
